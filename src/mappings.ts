...
export function handleProposalCreatedNewGovernor(event: ProposalCreated): void {
  let modifiedId = event.params.id + BIGINT_FIVE;
  let proposal = getOrCreateProposal(modifiedId.toString());
  let proposer = getOrCreateDelegate(
    event.params.proposer.toHexString(),
    false
  );
...
export function handleProposalCanceledNewGovernor(event: ProposalCanceled): void {
  let modifiedId = event.params.id + BIGINT_FIVE;
  let proposal = getOrCreateProposal(modifiedId.toString());
...
export function handleProposalExecutedNewGovernor(event: ProposalExecuted): void {
  let modifiedId = event.params.id + BIGINT_FIVE;
  let governance = getGovernanceEntity();
  let proposal = getOrCreateProposal(modifiedId.toString());
...
export function handleVoteCastNewGovernor(event: VoteCast): void {
  let modifiedId = event.params.proposalId + BIGINT_FIVE;
  let proposal = getOrCreateProposal(modifiedId.toString());
  let voteId = event.params.voter
    .toHexString()
    .concat("-")
    .concat(modifiedId.toString());
  let vote = getOrCreateVote(voteId);
  let voter = getOrCreateDelegate(event.params.voter.toHexString(), false);
